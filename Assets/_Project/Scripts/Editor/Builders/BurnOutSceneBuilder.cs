...
namespace BurnOut.Editor
{
    public static class BurnOutSceneBuilder
    {
        ...
        private static Slider CreateHudMeter(string name, Transform parent, Vector2 position, Vector2 size, Color fillColor) { var go = new GameObject(name, typeof(RectTransform), typeof(Image), typeof(Slider)); go.transform.SetParent(parent, false); var rect = go.GetComponent<RectTransform>(); rect.anchorMin = rect.anchorMax = new Vector2(0f, 1f); rect.pivot = new Vector2(0f, 1f); rect.anchoredPosition = position; rect.sizeDelta = size; var rounded = BurnOutSpriteFactory.GetRoundedBarSprite((int)size.y); var background = go.GetComponent<Image>(); background.sprite = rounded; background.type = Image.Type.Sliced; background.color = new Color(.04f, .02f, .08f, .72f); background.raycastTarget = false; var fill = new GameObject("Fill", typeof(RectTransform), typeof(Image)); fill.transform.SetParent(go.transform, false); var fillRect = fill.GetComponent<RectTransform>(); fillRect.anchorMin = Vector2.zero; fillRect.anchorMax = Vector2.one; fillRect.offsetMin = new Vector2(1f, 1f); fillRect.offsetMax = new Vector2(-1f, -1f); var fillImage = fill.GetComponent<Image>(); fillImage.sprite = rounded; fillImage.type = Image.Type.Sliced; fillImage.color = fillColor; fillImage.raycastTarget = false; var slider = go.GetComponent<Slider>(); slider.fillRect = fillRect; slider.direction = Slider.Direction.LeftToRight; slider.value = 1f; return slider; }
